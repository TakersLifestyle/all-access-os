...
// ── Branded fallback card (no image) ─────────────────────────────────────────

function BrandedFallbackCard({
  post,
  dateStr,
}: {
  post: SocialPost;
  ...
  return (
    <a
      ...
      className="group block rounded-2xl overflow-hidden border hover:border-pink-500/50 transition-all duration-300 hover:shadow-[0_0_32px_rgba(255,0,127,0.18)] hover:-translate-y-0.5"
      style={{
        background: isIG ? igGradient : ttGradient,
    ...
    >
      ...
      <div className="aspect-square relative flex flex-col items-center justify-center gap-3 px-4 overflow-hidden">
        ...
        <div
          ...
          style={{
            background: isIG
              ...
              : "radial-gradient(circle, #69c9d0 0%, transparent 70%)",
            top: "-20%",
            right: "-20%",
          }}
        />
        <div
          ...
          style={{
            background: isIG
              ...
              : "radial-gradient(circle, #ee1d52 0%, transparent 70%)",
            bottom: "-10%",
            left: "-10%",
          }}
        />
        ...
        <div className="absolute bottom-3 inset-x-3 flex items-center justify-between">
          ...
          <span
            className="text-[10px] font-bold px-2.5 py-1 rounded-lg border transition group-hover:scale-105"
            style={{
              color: isIG ? "#ff7eb8" : "#69c9d0",
              borderColor: isIG
                ? "rgba(255,0,127,0.25)"
                : "rgba(105,201,208,0.25)",
              background: isIG
                ? "rgba(255,0,127,0.08)"
                : "rgba(105,201,208,0.06)",
            }}
          >
            View Post →
          </span>
...
// ── Image card (has real image) ───────────────────────────────────────────────

function ImageCard({ post, dateStr }: { post: SocialPost; dateStr: string }) {
  const [errored, setErrored] = useState(false);
  const isIG = post.platform === "instagram";

  // If image errors out, fall back to branded card
  if (errored) {
    return <BrandedFallbackCard post={post} dateStr={dateStr} />;
  }
    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {Array.from({ length: Math.min(maxPosts, 4) }).map((_, i) => (
          <div
            key={i}
            className="aspect-square rounded-2xl border border-white/8 bg-white/[0.03] animate-pulse"
          />
        ))}
      </div>
    );
