    ...
    <nav className="border-b border-white/10 px-6 py-4">
      <div className="flex items-center justify-between">
          ...
          {user ? (
            ...
            <>
              ...
              <div className="flex items-center gap-3">
                <a href="https://www.instagram.com/allaccesswinnipeg/" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="text-white/30 hover:text-pink-500 transition">
                  <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                  </svg>
                </a>
                <a href="https://www.tiktok.com/@allaccesswinnipeg" target="_blank" rel="noopener noreferrer" aria-label="TikTok" className="text-white/30 hover:text-pink-500 transition">
              ...
              <Link href="/signup" className="bg-pink-600 hover:bg-pink-500 px-4 py-2 rounded-lg font-medium transition">
                Join Now
              </Link>
            </>
          )}
        ...
        <div className="flex md:hidden items-center gap-3">
          {!user && (
            <Link href="/events" onClick={closeMobile} className="bg-pink-600 hover:bg-pink-500 px-3 py-1.5 rounded-lg text-sm font-semibold transition">
              Get Tickets
            </Link>
          )}
          {isAdmin && (
            <Link href="/admin" onClick={closeMobile} className="text-amber-400 text-sm font-semibold">
              Admin
            </Link>
          )}
          <button
      ...
      {mobileOpen && (
        <div className="md:hidden pt-4 pb-2 border-t border-white/10 mt-4 flex flex-col gap-0.5">
          {user ? (
            <>
              ...
              <a href="https://www.instagram.com/allaccesswinnipeg/" target="_blank" rel="noopener noreferrer" onClick={closeMobile} className="px-3 py-3 text-sm text-white/60 hover:text-white hover:bg-white/5 rounded-xl transition flex items-center gap-2.5">
              ...
              <div className="h-px bg-white/5 my-2" />
              <MobileLink href="/profile" onClick={closeMobile}>
                {profile?.displayName ?? profile?.email ?? "Profile"}
              </MobileLink>
              <button
                onClick={handleSignOut}
                className="text-left px-3 py-3 text-sm text-white/40 hover:text-white hover:bg-white/5 rounded-xl transition"
              >
                Sign out
              </button>
            </>
            ...
            <>
              ...
              <MobileLink href="/connect" onClick={closeMobile}>Live Feed</MobileLink>
              <div className="h-px bg-white/5 my-2" />
              <MobileLink href="/login" onClick={closeMobile}>Log in</MobileLink>
              <Link
                href="/signup"
                onClick={closeMobile}
                className="mx-0 mt-1 block text-center bg-pink-600 hover:bg-pink-500 py-3 rounded-xl text-sm font-bold transition"
              >
                Join the Community
              </Link>
            </>
...
function MobileLink({ href, onClick, children }: { href: string; onClick: () => void; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className="px-3 py-3 text-sm text-white/70 hover:text-white hover:bg-white/5 rounded-xl transition"
    >
      {children}
    </Link>
  );
