// Called immediately from the Sunset Sessions success page after Stripe redirects back.
// Verifies payment, marks order paid, sends confirmation email — no webhook wait.
  ...
  try {
    ...
    if (toEmail) {
      await sendTicketConfirmation({
        ...
        toEmail,
        displayName: null,
        subject: (order.emailSubject as string | undefined) ?? "🎨 You're In — Sunset Sessions Vol. 01 | ALL ACCESS Sip & Paint Experience",
        accentColor: (order.emailAccentColor as string | undefined) ?? "#D4AF37",
        eventTitle: order.eventTitle as string,
        eventDate: "2026-07-31",
        eventLocation: "Premium Rooftop Venue, Winnipeg, MB (Location revealed 1 week before)",
        ...
        paidAt: new Date().toISOString(),
      }).catch((err) => console.error("[paint-sip-confirm] email error:", err));
    }
    ...
    console.error("[paint-sip-confirm] error:", err);
