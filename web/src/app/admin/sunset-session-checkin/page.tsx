...
const PAINT_SIP_ID = "paint-sip-rooftop-2026";
...
export default function SunsetSessionCheckinPage() {
  ...
  const startScanner = useCallback(async () => {
    ...
    const scanner = new Html5Qrcode("qr-reader-sip");
  ...
  const stopScanner = useCallback(async () => {
    ...
    if (s?.stop) { try { await s.stop(); } catch { /* ok */ } }
  ...
  const loadOrders = useCallback(async (q = "") => {
    ...
    try {
      const url = `/api/admin/event-orders?eventId=${PAINT_SIP_ID}${q ? `&search=${encodeURIComponent(q)}` : ""}`;
  ...
  return (
    <main className="min-h-screen bg-[#060606] text-white">
      ...
      <div className="border-b border-white/8 px-4 py-4 flex items-center justify-between">
        <div>
          <p className="text-[10px] text-[#D4AF37]/60 font-bold tracking-[0.2em] uppercase">ALL ACCESS</p>
          <h1 className="text-lg font-black tracking-tight">Door Check-In</h1>
          <p className="text-white/30 text-xs mt-0.5">Sunset Sessions Vol. 01 · Sip &amp; Paint · July 31, 2026</p>
        </div>
        {stats && (
          <div className="text-right">
            <p className="text-2xl font-black text-[#D4AF37]">{stats.checkedIn}</p>
        ...
        <div className="p-4 space-y-4 max-w-md mx-auto">
          {scanResult && (
            <div className={`rounded-2xl border p-5 text-center transition-all ${
              scanResult.valid ? "bg-emerald-950/60 border-emerald-500/40"
              : scanResult.reason === "already_used" ? "bg-orange-950/60 border-orange-500/40"
              ...
              <p className={`font-black text-lg mb-1 ${
                scanResult.valid ? "text-emerald-300"
                : scanResult.reason === "already_used" ? "text-orange-300"
                : "text-red-300"
              }`}>
                {scanResult.valid ? "Checked In!" : scanResult.reason === "already_used" ? "Already Scanned" : "Invalid Ticket"}
              </p>
              {scanResult.order && (
              ...
              <button onClick={() => setScanResult(null)} className="mt-3 text-white/25 text-xs hover:text-white/50 transition">Dismiss</button>
            </div>
          )}

          {!scanResult && (
            <div className="rounded-2xl border border-white/10 bg-black overflow-hidden relative">
              <div id="qr-reader-sip" ref={scannerDivRef} className="w-full aspect-square" style={{ minHeight: 280 }} />
              {!scannerActive && (
          ...
          {!scannerActive ? (
            <button
              ...
              className="w-full bg-[#D4AF37] hover:bg-[#c9a430] text-black font-black py-4 rounded-2xl text-base transition"
            >
              {scanLoading ? "Processing…" : "Start Camera Scanner"}
            </button>
          ) : (
            <button onClick={stopScanner} className="w-full border border-white/15 text-white/60 hover:text-white hover:border-white/30 font-bold py-3.5 rounded-2xl text-sm transition">
              Stop Scanner
            </button>
          )}
          ...
          <div className="flex gap-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && manualId.trim()) { verifyTicket(manualId.trim()); setManualId(""); }
              }}
              placeholder="Paste Order ID…"
              className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none focus:border-[#D4AF37]/40"
            />
            <button
              onClick={() => { if (manualId.trim()) { verifyTicket(manualId.trim()); setManualId(""); } }}
              disabled={!manualId.trim() || scanLoading}
              className="bg-[#D4AF37] hover:bg-[#c9a430] disabled:opacity-30 text-black font-black px-5 rounded-xl transition"
            >
              ✓
            </button>
          </div>
        ...
        <div className="p-4 space-y-4 max-w-2xl mx-auto">
          {stats && (
            <div className="space-y-2">
              <div className="bg-[#D4AF37]/10 border border-[#D4AF37]/30 rounded-xl p-4 flex items-center justify-between">
                <div>
                  <p className="text-[#D4AF37] text-xs font-bold tracking-widest uppercase">Still Expected</p>
                  <p className="text-[#D4AF37] text-4xl font-black leading-none mt-1">{stats.remaining}</p>
                  <p className="text-[#D4AF37]/50 text-xs mt-1">guests still on their way</p>
                </div>
          ...
          <div className="relative">
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none focus:border-[#D4AF37]/40 pr-10"
            />
            {search && (
              <button onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/25 hover:text-white/60 text-lg">×</button>
            )}
          </div>

          <div className="flex items-center justify-between">
            <p className="text-white/25 text-xs">{orders.length} result{orders.length !== 1 ? "s" : ""}</p>
            <button onClick={() => loadOrders(search)} disabled={listLoading} className="text-[#D4AF37]/60 hover:text-[#D4AF37] text-xs font-semibold transition">
              {listLoading ? "Loading…" : "↻ Refresh"}
            </button>
          </div>

          {listError && (
            <div className="bg-red-950/40 border border-red-800/40 rounded-xl px-4 py-3 text-red-400 text-sm">{listError}</div>
          )}
            ...
            {orders.map((o) => (
              <div
                ...
                className={`border rounded-xl px-4 py-3.5 flex items-center gap-3 transition ${
                  o.checkedIn ? "bg-emerald-950/30 border-emerald-600/30" : "bg-white/[0.02] border-white/8"
                }`}
              >
                <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center">
                  {o.checkedIn ? <span className="text-xl">✅</span> : <div className="w-3 h-3 rounded-full bg-white/15" />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-white/90 truncate">{o.userEmail || "Guest"}</p>
                  <p className="text-xs text-white/35 mt-0.5">{o.quantity} × {o.ticketTierName}</p>
                ...
                {!o.checkedIn ? (
                  <button
                    ...
                    className="flex-shrink-0 bg-[#D4AF37] hover:bg-[#c9a430] disabled:opacity-40 text-black text-xs font-black px-3 py-2 rounded-lg transition"
                  >
